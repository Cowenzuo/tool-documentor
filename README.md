...
- **当前版本**：`release/` 里的成品是 `0.1.2-alpha2`（免安装目录 `win-unpacked/` 与安装包，2026-09-22 打，
  对应提交 `eadaa97`）；另留着 `0.1.2-alpha1`、`0.1.1-alpha1`、`0.1.0-beta1`、`0.1.0-alpha3`、
  `0.1.0-alpha1` 五个更早的安装包（重新打包由用户明确要求时才做）。
  `0.1.2-alpha2` 相对 `0.1.2-alpha1` 多了这些：用户侧换类型（卡片头上换，内容能带的都带过去）、
  撤销的两处修复（挂起编辑先落定、撤销后界面按 store 覆盖回来）、缩表那句"无法撤销"的假提示退役，
  以及版本开发过程那一区改写成九份现役设计。两个版本都能打开现在的工程。
