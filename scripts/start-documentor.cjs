 ...
 *
 * ⚠️ `.dev-mode` 只对本脚本生效：走 `start-documentor.cmd` 或本脚本的启动都会读到它，
 *    指向那个 .cmd 的快捷方式也算。安装包建出来的快捷方式起的是装好的 Documentor.exe，
 *    不读这个标记，不受影响。共享机器上留着它，别人从仓库这边启动也会跑到开发版
 *    （还要现编库，等十几秒），所以开发完记得删掉：`del .dev-mode`。
 *    该文件已在 .gitignore 里，不会入库。
 *
