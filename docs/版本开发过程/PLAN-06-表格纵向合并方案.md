...
| 应用内 | 开关默认勾选、提示「已合并 2 处」、预览 `rowspan=[3,4]`、界面导出产物与 CLI 一致 |

## 8 后来的调整

原实现除勾选框外还有一个「补齐合并」按钮：把自动判定认出来的组合写成显式 `rowSpans`，
写之前弹一次确认。这一层是多余的中间态，已删除：

- 合成本来就是勾选框的能力，勾上即合并、取消即散开，不需要第二次操作；
- 当时加确认是为了防"合并清空单元格"的老毛病，而现在的合并只调换显示，不碰 `data`，
  加上撤销已经落地（见 `PLAN-10-撤销与重做方案.md`），误操作可以退回去；
- 连带删掉 `completeRowSpans` 与 `inferRowSpansFromData` 两个纯函数：界面不再写显式跨度，
  它们没有调用方。`rowSpans` 保留为接手外部数据的形态，缩表时仍按新尺寸裁剪。

判定口径本身没变，仍是两种来源取并集，见 `docs/WORD解压结构研究/表格与合并.md` 第 4 节。
