...
| 编号 | 落实方式 | 回归 |
| --- | --- | --- |
| S5 | miniFAT 的未使用槽改写 `FREESECT`，与 FAT 同口径；FAT 扇区数超过头部 109 个 DIFAT 槽时直接报错，不再静默产出非法容器 | postprocess `cfb.test.ts` 新增 5 条，含 109 可写出、110 报错的边界 |
| S6 | 题注文字一律原样带出，编号归样式或题注域，剥离子号的做法连同 `core/captions.ts` 一并删除；示例模板与样例数据里的手写编号同时去掉 | `docx` 题注相关期望随口径更新，示例模板的期望同步 |
| S8 | 图表降级占位段整段带出源码；超过 2000 字符才截断，且切在行尾并写明已截断与真实长度 | docx `serializer.test.ts`、`writer.test.ts` 新增 3 条 |
| S9 | 章节号按当前路径上实际存在的标题层级拼，缺层不补 0；副标题不进编号链；没有可挂靠的标题时退化为不带章节号的静态题注并给一条警告 | docx `serializer.test.ts`、`writer.test.ts` 新增 5 条 |
| 锁定写入侧 | `removeBlock`、`moveBlock`、`updateBlock`、`importImage` 按档位拒绝：`keep` 与 `readonly` 不能删也不能挪，`readonly` 还不能改内容与换图；落库的档位以库里的块为准；界面同步把与锁定块相邻那一侧的上下移按钮置灰并写明原因 | desktop 类型检查与 `pnpm e2e` 新增 7 条断言 |

### 10.2 门禁与交付

| 编号 | 落实方式 | 回归 |
| --- | --- | --- |
| G2 | 产物级 `dist` 一侧的覆盖边界与手工验证要求写进 `docs/AI开发约定/项目特定规范.md` 第 5 节 | 文档口径 |
| G6 | `README.md`、`项目特定规范.md`、`packages/desktop/electron-builder.yml`、`scripts/start-documentor.cjs` 按实际订正：`.dev-mode` 只对一键启动生效，打包路径基准是进程工作目录，`verify` 与 `verify:local` 的差别写明 | 文档口径 |
| 交付观感 | 应用图标落到 `packages/desktop/build/icon.ico`，含 16 到 256 共 7 个尺寸 | Windows 图标加载器逐个尺寸读回 |
| E2E 时序 | 探针等标题入库改为轮询，去掉固定 1000 毫秒停顿带来的偶发失败 | `pnpm e2e` 连跑 |

alpha4 收口时：`pnpm verify` 退出码 0，`pnpm -r test` 211 通过加 1 跳过，`pnpm e2e` 全绿。

## 11 仍未处理

- **发布版由用户要求时才重建**：`release/` 现为 `0.1.0-beta1`，从 `57b056c` 打包，
  打包后跑 `node scripts/verify-package.cjs` 复核分发边界；alpha1 与 alpha3 的安装包保留；
- 应修 9 条已经全部落地：S1 随表格合并语义一并处理，S2 到 S4 与 S7 在 alpha4 前两批，
  S5、S6、S8、S9 在本批次，见第 10 节；
